...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=2300, input_size=32, width_mult=1.):
        ...
        # building classifier
        self.classifier_layer1 = nn.Linear(self.last_channel, 2300)
        self.bnorm1 = nn.BatchNorm1d(2300)
        self.classifier_layer2 = nn.Linear(2300, 2300)
...
def learn(model, data_loader, test_loader, scheduler):
    for epoch in range(numEpochs):
        ...
        torch.save(model.state_dict(), './modelHyperparameters_verify_model.pt')
        end_time = time.time()
        print('Epoch: {}\tTime: {}'.format(epoch+1,end_time-start_time))
        #save model
        # torch.save(model, './modelHyperparameters_verify_mobileNetv3.pt')
        torch.save(model.state_dict(), './modelHyperparameters_verify_mobileNetv3_model.pt')
